...
void monty(argst_r *args)
{
	size_t len = 0;
	int get = 0;
	void (*code_func)(stackt_r **, unsigned int);
	...
	while (1)
	{
		...
		if (!code_func)
		{
			dprintf(STDERR_FILENO, UNKNOWN, args->line_numb, data.words[0]);
			free_all(1);
			exit(EXIT_FAILURE);
		}
		code_func(&(data.stack), args->line_numb);
		free_all(0);
	}
	free_all(1);
}
